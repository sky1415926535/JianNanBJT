...
> 当前版本：**v4.2.1** — 宁波府切换支持 + 退出弹窗自动处理 + _is_on_big_map OCR辅助检测
...
| 🆕 **退出弹窗处理** | v4.2.1 新增：自动检测并取消退出游戏弹窗，防止误退出 |
| 🗺️ **宁波府支持** | v4.2.1 新增：宁波府坐标标定完成，支持自动切换 |
...
### v4.2.1（2026-06-13）
- ★ **宁波府切换支持**：完成宁波府坐标标定（x:1527, y:762），支持自动切换
- ★ **退出弹窗自动处理**：`_exit_big_map()` 新增退出游戏弹窗检测与取消（点击"再玩一会"）
- ★ **`_is_on_big_map` OCR 辅助检测**：新增策略0，通过识别"府印"/"营造"/"任务"等关键词辅助判定界面
- ★ **坐标模式兜底**：未配置州府时允许走 OCR 搜索模式（不再直接报错）
- ★ **已知问题修复**：RapidOCR 返回 None 时 `len()` 异常处理
- 更新 `config.json`：新增宁波府配置，默认目标改为宁波府

### v4.2（2026-06-13）
- ★ **所有府大地图坐标OCR自动标定完成**（8/8）：应天府/苏州府/杭州府/松江府/徽州府/扬州府/绍兴府/宁波府
